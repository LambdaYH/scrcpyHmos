...
// SIGNATURE_PADDING - 完全按照原版AdbKeyPair.java
...
std::vector<uint8_t> AdbKeyPair::signPayload(const uint8_t* payload, size_t payloadLen) {
    ...
    // 合并padding和payload
    std::vector<uint8_t> combined(SIGNATURE_PADDING_LEN + payloadLen);
    ...
    // ADB使用RSA/NoPadding (raw RSA with DECRYPT_MODE = private key operation)
    OH_CryptoAsymCipher* cipher = nullptr;
    OH_Crypto_ErrCode err = OH_CryptoAsymCipher_Create("RSA|NoPadding", &cipher);
    if (err != CRYPTO_SUCCESS) {
        OH_LOG_ERROR(LOG_APP, "AdbKeyPair: create cipher failed: %{public}d", err);
        return std::vector<uint8_t>(256, 0);
    }

    // DECRYPT_MODE corresponds to private key operation in ADB signing
    ...
    if (err != CRYPTO_SUCCESS) {
        OH_LOG_ERROR(LOG_APP, "AdbKeyPair: cipher final failed: %{public}d", err);
        return std::vector<uint8_t>(256, 0);
    }
